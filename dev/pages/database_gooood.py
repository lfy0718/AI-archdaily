...
def main():
    st.markdown("# Gooood 数据库管理")
    b.template_mongodb_connection_region(user_settings.mongodb_gooood_db_name,
                                         lambda db_name: setattr(user_settings, 'mongodb_gooood_db_name', db_name))
...
def _step1_upload_content():
    ...
    b.template_start_work_with_progress("上传项目", "DBStep1-upload",
                                        b.gooood__upload_content, skip_exist,
...
def _step2_calculating_embedding():
    ...
    def _plan1_region():
        ...
        b.template_start_work_with_progress("计算嵌入向量(使用multimodal_embedding_v1)", "DBStep2-calculate1",
                                            b.gooood__calculate_text_embedding_using_multimodal_embedding_v1_api,
                                            st_show_detail_number=True, st_show_detail_project_id=True,
    ...
    def _plan2_region():
        ...
        b.template_start_work_with_progress("计算嵌入向量(使用gme-Qwen2-VL-2B-Instruct)", "DBStep2-calculate2",
                                            b.gooood__calculate_text_embedding_using_gme_Qwen2_VL_2B_api,
                                            st_show_detail_number=True, st_show_detail_project_id=True,
                                            st_button_icon="✨", ctx_enable_ctx_scope_check=True)
    if _plan == "**方案1**":
        _plan1_region()
